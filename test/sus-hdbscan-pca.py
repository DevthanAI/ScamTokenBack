...
hbd_pca = pickle.load(open('pickle/hdbscan-pca.pkl', 'rb'))
...
# apply HDBSCAN to predict the clusters
y_hdb_pca = hbd_pca.fit_predict(x_pca)

# add the clusters to the dataset
data['Cluster'] = y_hdb_pca

# calculate the percentage of tokens in each cluster
print(data['Cluster'].value_counts(normalize=True))

# save the dataset
data.to_csv('csv/sus-hdbscan-pca.csv', index=False)
